...
class SentiText(object):
    def __init__(self, text):
        self.REGEX_REMOVE_PUNCTUATION = re.compile('[%s]' % re.escape(string.punctuation))
...
class SentimentIntensityAnalyzer(object):
    def __init__(self, lexicon_file="vader_lexicon.txt", emoji_lexicon="emoji_utf8_lexicon.txt"):
        ...
        self.SENTIMENT_LADEN_IDIOMS = {"cut the mustard": 2, "hand to mouth": -2,
        ...
        self.SPECIAL_CASE_IDIOMS = {"the shit": 3, "the bomb": 3, "bad ass": 1.5, "yeah right": -2,
                                    "kiss of death": -1.5}
    ...
    def polarity_scores(self, text):
        ...
        for item in words_and_emoticons:
            ...
            if item.lower() in self.BOOSTER_DICT:
    ...
    def _special_idioms_check(self, valence, words_and_emoticons, i):
        ...
        for n_gram in n_grams:
            if n_gram in self.BOOSTER_DICT:
                valence = valence + self.BOOSTER_DICT[n_gram]
        return valence

    def _sentiment_laden_idioms_check(self, valence, senti_text_lower):
        # Future Work
        # check for sentiment laden idioms that don't contain a lexicon word
        idioms_valences = []
        for idiom in self.SENTIMENT_LADEN_IDIOMS:
            if idiom in senti_text_lower:
                print(idiom, senti_text_lower)
                valence = self.SENTIMENT_LADEN_IDIOMS[idiom]
